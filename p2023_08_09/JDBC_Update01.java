...
class JDBC_Update01 {
	public static void main(String[] args) {
		...
		try {
			...
			sql = "UPDATE customer SET name=?,email = ?, tel = ? where no = ?";// 값이 들어갈 곳은 ?로 설정
			
			pstmt = con.prepareStatement(sql);// ?자리에 값을 설정하기 위해 sql뮨울 앍어옴
			
			// varvhar2는 setString으로 설정, no는 setInt로 설정
			pstmt.setString(1, name);
			pstmt.setString(2, email);
			pstmt.setString(3, tel);
			pstmt.setInt(4, ino);
			int result = pstmt.executeUpdate();// executeUpdate로 실행
